...
try:
    ...
    _r = redis.from_url(_redis_url, decode_responses=False)
    _r.ping()
...
def _df_to_bytes(df: pd.DataFrame) -> bytes:
    return df.to_json(date_format="iso").encode()


def _bytes_to_df(raw: bytes) -> pd.DataFrame:
    return pd.read_json(raw.decode())
...
def get_daily(ticker: str, period: str = "10d") -> pd.DataFrame:
    ...
    df = yf.download(ticker, interval="1d", period=period, auto_adjust=True)
    if not df.empty:
...
def get_cached_indicator(name: str, ticker: str, interval: str,
...
def set_cached_indicator(name: str, ticker: str, interval: str,
                         period: str, payload: dict) -> None:
    key = _cache_key("ind", name, ticker, interval, period)
...
def get_cached_optimization(ticker: str, interval: str,
                            period: str) -> dict | None:
...
def set_cached_optimization(ticker: str, interval: str,
                            period: str, result: dict) -> None:
    key = _cache_key("opt", ticker, interval, period)
