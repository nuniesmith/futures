...
def estimate_full_run() -> None:
    ...
    info(f"Estimated time: 2-6 hours (depends on bar loading speed + GPU)")
    info("")
...
def main() -> int:
    ...
    if _FAIL > 0:
        ...
        if not any(bars_loaded.values()):
            ...
            print(f"    1. Start data service: docker compose up -d postgres redis data")
            print(f"    2. Set ENGINE_DATA_URL: export ENGINE_DATA_URL=http://<server-tailscale-ip>:8050")
            print(f"    3. Re-run: python scripts/smoke_test_dataset.py")
            print()
        return 1
    elif _WARN > 0:
        ...
        print(f"    python -m lib.services.training.dataset_generator \\")
        print(f"      --symbols {','.join(ALL_25_SYMBOLS)} \\")
        print(f"      --days 120 \\")
        print(f"      --breakout-type all \\")
        print(f"      --orb-session all")
        print()
        print(f"  {_DIM}Or trigger via the trainer API:{_NC}")
        print(f"    curl -X POST http://<trainer>:8200/train \\")
        print(f'      -H "Content-Type: application/json" \\')
        print(f'      -d \'{{"days_back": 120, "epochs": 80, "patience": 15}}\'')
        print()
        return 0
