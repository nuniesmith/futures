...
Public API:
    from backfill import run_backfill, get_backfill_status, get_stored_bars
...
def _is_postgres() -> bool:
    ...
    try:
        from models import _is_using_postgres
...
def _fetch_chunk_massive(
    ...
    """
    try:
        from cache import _get_massive_provider

        provider = _get_massive_provider()
        if provider is None or not provider.is_available:
            return pd.DataFrame()

        from massive_client import _dropna_ohlc, _parse_timestamp_index
...
def _fetch_chunk_yfinance(
    ...
    try:
        ...
        from cache import _flatten_columns
...
def _store_bars(conn, symbol: str, df: pd.DataFrame, interval: str = "1m") -> int:
    ...
    for idx, row in df.iterrows():
        ...
        rows.append((symbol, ts, o, h, l, c, v, interval))
...
def _compute_date_range(
    ...
    if latest_ts:
        try:
            ...
            if start_dt >= end_dt:
                return end_dt, end_dt  # Nothing to fetch
...
def get_gap_report(
    ...
    trading_days = min(days_back, (df.index.max() - df.index.min()).days + 1)
