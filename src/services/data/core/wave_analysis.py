...
def _detect_waves(
    ...
    c_smooth = close_s.ewm(alpha=1.0 / 10.0, adjust=False).mean().values
...
def _detect_market_phase(
    ...
    wider_high = np.max(high[-20:]) if len(high) >= 20 else recent_high
    wider_low = np.min(low[-20:]) if len(low) >= 20 else recent_low
...
def calculate_wave_analysis(
    ...
    ao = (hl2_series.rolling(5).mean() - hl2_series.rolling(34).mean()).fillna(0).values
