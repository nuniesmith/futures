...
Usage:
    python -m futures_lib.services.engine.main

Docker:
    CMD ["python", "-m", "futures_lib.services.engine.main"]
"""
...
def _handle_check_orb(engine) -> None:
    ...
    try:
        from src.futures_lib.services.engine.orb import (
        ...
        for asset in assets:
            ...
            try:
                ...
                if result.breakout_detected:
                    ...
                    # Send alert
                    try:
                        from src.futures_lib.core.alerts import send_signal
