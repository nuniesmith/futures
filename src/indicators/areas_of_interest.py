...
def identify_fair_value_gaps(df, gap_threshold=0.0005):
    ...
    try:
        ...
        result.loc[bullish_fvg, "fvg_level"] = (
        ...
        result.loc[bearish_fvg, "fvg_level"] = (
        ...
        result.loc[small_gaps, ["bullish_fvg", "bearish_fvg", "fvg_level"]] = [
            False,
            False,
            np.nan,
        ]
...
def identify_supply_demand_zones(
    df, zone_length=5, zone_strength_threshold=3, lookforward_window=20
):
    """
    ...
    try:
        ...
        for i in range(zone_length, len(result)):
            ...
            touches_mask = np.zeros(
                min(lookforward_window, len(result) - i), dtype=bool
            )
...
def identify_key_levels(df, round_digits=2):
    ...
    try:
        ...
        if "close" in result.columns and result["close"].median() > 1000:
            ...
            near_thousands = (
                np.abs(prices - thousands_level) / prices < 0.005
            )  # Within 0.5%
            ...
            if result["close"].median() > 5000:  # Likely BTC
                ...
                near_ten_thousands = (
                    np.abs(prices - ten_thousands_level) / prices < 0.01
                )  # Within 1%
...
def identify_session_levels(df, time_column="datetime"):
    ...
    try:
        ...
        if time_column in result.columns:
            ...
            try:
                time_data = pd.Series(
                    pd.to_datetime(result.index, errors="coerce"), index=result.index
                )
            except Exception:
        ...
        try:
            ...
            result.loc[
                (hour_data == ny_midnight_hour) & (minute_data == 0), "is_ny_midnight"
            ] = True
            ...
            if not result_clean.empty:
                ...
                weekly_high_idx = result_clean.groupby(["temp_year", "temp_week"])[
                    "high"
                ].idxmax()
                weekly_low_idx = result_clean.groupby(["temp_year", "temp_week"])[
                    "low"
                ].idxmin()
        ...
        result = result.drop(
            columns=["temp_date", "temp_year", "temp_week"], errors="ignore"
        )
    ...
    try:
        ...
        if all(col in df.columns for col in ["supply_zone", "zone_low", "zone_high"]):
            ...
            if not supply_df.empty:
                ...
                in_supply = np.any(
                    (supply_df["zone_low"] <= current_price)
        ...
        if all(col in df.columns for col in ["demand_zone", "zone_low", "zone_high"]):
            ...
            if not demand_df.empty:
                ...
                in_demand = np.any(
                    (demand_df["zone_low"] <= current_price)
        ...
        if "key_level" in df.columns:
            ...
            if not key_level_df.empty:
                ...
                at_key_level = np.any(
                    np.abs(key_level_df["close"] - current_price) < buffer
                )
        ...
        for col in session_columns:
            if col in df.columns:
                ...
                if not session_df.empty:
                    ...
                    at_session_level = np.any(
                        np.abs(session_df["close"] - current_price) < buffer
                    )
...
def identify_bitcoin_specific_levels(df, fibonacci_base=None):
    ...
    try:
        ...
        for price in halving_prices:
            ...
            is_near_halving_price = (result["high"] >= price - price_buffer) & (
        ...
        if fibonacci_base is not None:
            ...
            for level in fib_levels:
                ...
                is_near_fib_level = (result["high"] >= level - level_buffer) & (
