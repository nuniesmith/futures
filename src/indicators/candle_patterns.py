...
def identify_manipulation_candles(
    ...
    try:
        ...
        bullish_condition = (result["low"] < result["low"].shift(1)) & (
        ...
        bearish_condition = (result["high"] > result["high"].shift(1)) & (
        ...
        if enhanced_detection:
            ...
            result["bullish_pinbar"] = (
                ...
                & (
                    result["lower_wick_ratio"] > result["upper_wick_ratio"] * 2
                )  # Lower wick much larger than upper
            ...
            result["bearish_pinbar"] = (
                ...
                & (
                    result["upper_wick_ratio"] > result["lower_wick_ratio"] * 2
                )  # Upper wick much larger than lower
    ...
    try:
            ...
            if timeframe == "D1" or timeframe == "Daily":
                ...
                if "candle_height_pct" in result.columns:
                    ...
                    strong_signals = strong_signals & (
                        result["candle_height_pct"] > avg_height * 1.5
                    )
                ...
                if "candle_height_pct" in result.columns and enhanced_filtering:
                    very_strong = result["candle_height_pct"] > result[
                        "candle_height_pct"
                    ].quantile(0.85)
                ...
                result["valid_signal"] = False
                logger.info(
                    "no_valid_signals_for_timeframe",
                    timeframe=timeframe,
                    mode="standard",
                )
        ...
        if enhanced_filtering:
            ...
            if "candle_height_pct" in result.columns:
                ...
                max_height = result["candle_height_pct"].quantile(
                    0.95
            ...
            if "bullish_pinbar" in result.columns:
                ...
                result.loc[
                    result["bullish_pinbar"] | result["bearish_pinbar"],
                    "signal_strength",
                ] += 0.2
            ...
            if "bullish_engulfing" in result.columns:
                ...
                result.loc[
                    result["bullish_engulfing"] | result["bearish_engulfing"],
                    "signal_strength",
                ] += 0.3
    ...
    try:
        ...
        if bullish_entries.any():
            ...
            if dynamic_targets:
                ...
                result.loc[bullish_entries, "stop_loss"] = (
                    result.loc[bullish_entries, "low"]
                    - atr_multiplier * result.loc[bullish_entries, "atr"]
                )
            ...
            for rr in [1.0, 1.5, 2.0, 3.0]:
                ...
                result.loc[bullish_entries, f"tp_rr_{rr}"] = result.loc[
                    bullish_entries, "entry_price"
                ] + (risk * rr)
                ...
                if dynamic_targets and "candle_height_pct" in result.columns:
                    ...
                    if volatile_idx.any():
                        ...
                        result.loc[volatile_idx, f"tp_rr_{rr}"] = result.loc[
                            volatile_idx, "entry_price"
        ...
        if bearish_entries.any():
            ...
            if dynamic_targets:
                ...
                result.loc[bearish_entries, "stop_loss"] = (
                    result.loc[bearish_entries, "high"] + sl_distance
                )
                ...
                too_far_sl = (
                    result.loc[bearish_entries, "stop_loss"]
                    - result.loc[bearish_entries, "entry_price"]
                ) > max_distance
                ...
                # Standard fixed ATR-based stop loss
                result.loc[bearish_entries, "stop_loss"] = (
                    result.loc[bearish_entries, "high"]
                    + atr_multiplier * result.loc[bearish_entries, "atr"]
                )
            ...
            for rr in [1.0, 1.5, 2.0, 3.0]:
                ...
                # Standard take profit calculation
                result.loc[bearish_entries, f"tp_rr_{rr}"] = result.loc[
                    bearish_entries, "entry_price"
                ] - (risk * rr)
...
def identify_advanced_patterns(
    ...
    try:
        ...
        result["inside_bar"] = (result["high"] < result["high"].shift(1)) & (
            result["low"] > result["low"].shift(1)
        )

        # Identify outside bars (current bar's range engulfs previous bar's range)
        result["outside_bar"] = (result["high"] > result["high"].shift(1)) & (
            result["low"] < result["low"].shift(1)
        )
        ...
        if len(result) >= 4:
            ...
            highs = (
                result["high"]
                .rolling(window=4)
                .apply(lambda x: x[1] > x[0] and x[2] > x[1] and x[3] < x[2], raw=True)
            )
