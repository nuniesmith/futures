...
class IndicatorManager:
    ...
    def __init__(self, indicators: list[Indicator | dict[str, Any]] | None = None):
        ...
        self.logger.info(
            "indicator_manager_initialized", indicator_count=len(self._indicators)
        )
    ...
    def _add_indicator_internal(
        self, indicator: Indicator | dict[str, Any], groups: list[str] | None = None
    ) -> str:
        """
        ...
        # Handle indicator configuration
            ...
            try:
                ...
                self.logger.error(
                    "failed_to_create_indicator_from_config", error=str(e)
                )
                raise
        else:
            raise ValueError(
                "Indicator must be an Indicator instance or a configuration dictionary"
            )
    ...
    def add_group(
        self, group_name: str, indicator_ids: list[str] | None = None
    ) -> None:
        """
    ...
    def calculate_all(self, data: pd.DataFrame) -> dict[str, pd.DataFrame]:
        ...
        for ind_id, indicator in self._indicators.items():
            try:
                ...
                self.logger.error(
                    "indicator_calculation_failed", indicator_id=ind_id, error=str(e)
                )
                results[ind_id] = None
    ...
    def calculate_group(
        self, group_name: str, data: pd.DataFrame
    ...
    def calculate_indicators(
        self, indicator_ids: list[str], data: pd.DataFrame
    ) -> dict[str, pd.DataFrame]:
        """
    ...
    def get_group_values(
        ...
        for ind_id in self._groups[group_name]:
            if (
                ind_id in self._indicators
                and self._indicators[ind_id].values is not None
            ):
                try:
        ...
        for ind_cls in technical_indicators:
            ...
            if ind_category == category:
                try:
                    ...
                    self.logger.error(
                        "failed_to_add_indicator",
                        indicator_class=ind_cls.__name__,
                        error=str(e),
                    )

        self.logger.info(
            "added_indicators_by_category",
            count=len(added_indicators),
            category=category,
        )
        return added_indicators

    def create_combined_dataframe(
        self, data: pd.DataFrame, indicator_ids: list[str] | None = None
    ) -> pd.DataFrame:
        """
        ...
        for ind_id in indicator_ids:
            if ind_id in self._indicators:
                try:
                    ...
                    self.logger.debug(
                        "added_indicator_to_combined_dataframe", indicator_id=ind_id
                    )
                except Exception as e:
                    self.logger.error(
                        "indicator_calculation_failed",
    ...
    def create_batch_indicator_calculation(
        ...
        for ind in temp_indicators:
            try:
                ...
                self.logger.debug(
                    "added_indicator_to_result_dataframe", indicator_name=ind.name
                )
            except Exception as e:
                self.logger.error(
                    "indicator_calculation_failed",
