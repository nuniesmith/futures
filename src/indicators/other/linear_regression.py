...
class LinearRegressionIndicator:
    ...
    def _calculate_lr_slope(self, data: pd.Series) -> pd.Series:  # type: ignore[type-arg]
        ...
        slopes = data.rolling(window=self.period).apply(
            lambda y: np.polyfit(X, y, 1)[0] if len(y) == self.period else np.nan,
            raw=True,
        )
    ...
    def update(self, data_point: dict) -> float | None:
        ...
        try:
            ...
            if len(self.history_df) >= self.period:
                lr_slope_series = self._calculate_lr_slope(
                    self.history_df["Close"].copy()
            ...
            self.logger.error(
                "linear_regression_slope_update_failed", error=str(e), exc_info=True
            )
            return None
    ...
    def apply(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.error(
                "error_applying_linear_regression_slope", error=str(e), exc_info=True
            )
            raise
