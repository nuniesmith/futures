...
class ParabolicSARIndicator:
    ...
    def _calculate_sar(self, data: pd.DataFrame) -> pd.Series:
        ...
        self.logger.info(
            "calculating_parabolic_sar", step=self.step, max_step=self.max_step
        )
        ...
        ep = data["Low"].iloc[
            0
        ]  # Extreme Point; for an uptrend, EP is the highest high
    ...
    def update(self, data_point: dict) -> float | None:
        ...
        try:
            ...
            self.logger.error(
                "parabolic_sar_update_failed", error=str(e), exc_info=True
            )
            return None
    ...
    def apply(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.error(
                "error_applying_parabolic_sar", error=str(e), exc_info=True
            )
            raise
