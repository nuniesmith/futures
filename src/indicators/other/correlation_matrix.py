...
class CorrelationMatrixIndicator:
    ...
    def __init__(self, indicators: list):
        ...
        if not isinstance(indicators, list) or not all(
            isinstance(ind, str) for ind in indicators
        ):
        ...
        self.history_df = pd.DataFrame(
            columns=pd.Index(self.indicators, name="indicator")
        )
    ...
    def _calculate_correlation_matrix(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if missing_cols:
                raise ValueError(
                    f"DataFrame is missing the following columns: {', '.join(missing_cols)}"
                )
            ...
            if len(valid_data) < 2:
                raise ValueError(
                    "Not enough valid data points to calculate correlation matrix."
                )

            self.logger.info(
                "calculating_correlation_matrix", indicators=self.indicators
            )
    ...
    def update(self, data_point: dict) -> pd.DataFrame | None:
        ...
        try:
            ...
            if len(self.history_df.dropna()) >= 2:
                self.current_matrix = self._calculate_correlation_matrix(
                    self.history_df
                )
            else:
            ...
            self.logger.error(
                "correlation_matrix_update_failed", error=str(e), exc_info=True
            )
            return None
