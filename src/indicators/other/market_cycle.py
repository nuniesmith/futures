...
class MarketCycleIndicator:
    ...
    def _detect(self, data: pd.DataFrame) -> pd.Series:
        ...
        if bool(data["Close"].isnull().any()):
            self.logger.warning(
                "missing_values_detected", column="Close", action="forward_fill"
            )
        ...
        cycle.loc[(cycle.shift(1) == "Markdown") & (cycle != "Markdown")] = (
            "Accumulation"
        )
        ...
        self.logger.debug(
            "market_cycles_detected",
            cycle_counts=cycle.value_counts(dropna=True).to_dict(),
        )
        return cycle
        ...
        try:
            ...
            if close is None:
                self.logger.warning(
                    "missing_data_point",
                    field="close",
                    indicator="MarketCycleIndicator",
                )
                return None
