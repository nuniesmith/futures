...
class Stochastic(Indicator):
    ...
    def calculate(
        ...
        result = pd.DataFrame(
            {f"{self.name}_K": k, f"{self.name}_D": d}, index=data.index
        )
...
class StochasticRSI(Indicator):
    ...
    def __init__(
        self, name: str = "StochasticRSI", params: dict[str, Any] | None = None
    ):
        """
    ...
    def calculate(
        ...
        rs = pd.Series(
            rs.values if hasattr(rs, "values") else rs, index=data.index, dtype=float
        )
