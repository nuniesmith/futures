...
class RSI(Indicator):
    ...
    def calculate(
        ...
        avg_gain: pd.Series = pd.Series(
            gain.rolling(window=self.period).mean(), index=data.index
        )
        avg_loss: pd.Series = pd.Series(
            loss.rolling(window=self.period).mean(), index=data.index
        )
