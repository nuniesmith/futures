...
def is_active_session(
    ...
    try:
        ...
        if session_times is None:
            if asset_type.lower() in ["btc", "bitcoin", "crypto", "eth", "ethereum"]:
                ...
                session_times.update(
                    {
                        "crypto_high": CRYPTO_SESSIONS.get(
                            "high_volume", {"start": time(14, 0), "end": time(22, 0)}
                        ),
                        "crypto_low": CRYPTO_SESSIONS.get(
                            "low_volume", {"start": time(0, 0), "end": time(6, 0)}
                        ),
                    }
                )
        ...
        if asset_type.lower() in ["btc", "bitcoin", "crypto", "eth", "ethereum"]:
            result.update(
                {"in_crypto_high_volume": False, "in_crypto_low_volume": False}
            )
        ...
        for session_name, session_info in session_times.items():
            if "start" in session_info and "end" in session_info:
                ...
                if session_key in result:
                    result[session_key] = _is_time_in_session(
                        current_time, session_start, session_end
                    )

        # Determine optimal trading time
        result["is_optimal_trading_time"] = (
            result["in_london_session"] or result["in_ny_session"]
        )

        # For crypto, also consider crypto-specific high volume periods
        if (
            asset_type.lower() in ["btc", "bitcoin", "crypto", "eth", "ethereum"]
            and "in_crypto_high_volume" in result
        ):
            result["is_optimal_trading_time"] = (
                result["is_optimal_trading_time"] or result["in_crypto_high_volume"]
            )
        ...
        if (
            ...
            result["is_high_volume_session"] = (
                result["is_high_volume_session"] or result["in_crypto_high_volume"]
            )
...
def _is_time_in_session(
    current_time: time, session_start: time, session_end: time
) -> bool:
    """
    ...
    try:
        ...
        try:
            ...
            if time_column in df_copy.columns:
                df_copy["datetime"] = pd.to_datetime(
                    df_copy[time_column], errors="coerce"
                )
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error(
                            "all_datetime_extraction_attempts_failed", error=str(e2)
                        )
                        return pd.DataFrame(
                            {
                                "session": [
        ...
        for session_name, session_info in sessions.items():
            ...
            if start_hour < end_hour:
                df_copy[f"is_{session_name}"] = df_copy["hour"].between(
                    start_hour, end_hour, inclusive="left"
                )
            else:
                df_copy[f"is_{session_name}"] = ~df_copy["hour"].between(
                    end_hour, start_hour, inclusive="both"
                )

        # Define 'Other' as not in any regular session (excluding crypto sessions)
        regular_sessions = ["asian", "london", "ny", "overlap"]
        df_copy["is_other"] = ~df_copy[[f"is_{s}" for s in regular_sessions]].any(
            axis=1
        )
        ...
        if asset_type.lower() in ["btc", "bitcoin", "crypto", "eth", "ethereum"]:
            session_mapping.extend(
                [
                    ("Crypto High Volume", "is_crypto_high"),
                    ("Crypto Low Volume", "is_crypto_low"),
                ]
            )
                        ...
                        if isinstance(bullish_df, pd.DataFrame) and len(bullish_df) > 0:
                            ...
                            for idx in bullish_df.index:  # type: ignore[union-attr]
                                if idx in df.index:
                                    ...
                                    idx_pos = (
                                        int(_loc)
                                        if isinstance(_loc, (int, np.integer))
                                        else -1
                                    )
        ...
        if asset_type.lower() in ["btc", "bitcoin", "crypto", "eth", "ethereum"]:
            ...
            if "avg_volatility" in result_df.columns:
                result_df["volatility_rank"] = result_df["avg_volatility"].rank(
                    ascending=False
                )
            ...
            if (
                "avg_volatility" in result_df.columns
                and "bullish_mc_success_rate" in result_df.columns
            ):
                ...
                good_sessions = (result_df["avg_volatility"] > median_vol) & (
                    (result_df["bullish_mc_success_rate"] > 0.5)
                    | (result_df["bearish_mc_success_rate"] > 0.5)
                )
    ...
    try:
        ...
        if target_session.startswith("crypto_"):
            if target_session == "crypto_high":
                session_start = CRYPTO_SESSIONS.get(
                    "high_volume", {"start": time(14, 0)}
                )["start"]
            elif target_session == "crypto_low":
                session_start = CRYPTO_SESSIONS.get(
                    "low_volume", {"start": time(0, 0)}
                )["start"]
            else:
                logger.warning(
                    "unknown_crypto_session",
            ...
            if target_session not in DEFAULT_SESSIONS:
                logger.warning(
                    "unknown_session", target_session=target_session, fallback="london"
                )
        ...
        session_start_today = datetime.combine(
            today, session_start, tzinfo=pytz.timezone(tz)
        )
        ...
        session_start_tomorrow = datetime.combine(
            tomorrow, session_start, tzinfo=pytz.timezone(tz)
        )
...
def get_current_session(
    timestamp: pd.Timestamp | datetime | None = None, asset_type: str = "forex"
) -> str:
    """
...
def add_session_indicators(
    ...
    try:
        ...
        _ts_for_hour = (
            ...
            else pd.Series(time_series, index=result.index)
        )
        ...
        for session_name, session_info in sessions.items():
            ...
            if start_hour < end_hour:
                ...
                # Overnight session
                result[col_name] = ~hour_series.between(
                    end_hour, start_hour, inclusive="both"
                )

        # Add convenience flags for optimal trading times and high volume sessions
        result["is_optimal_trading_time"] = (
            result["in_london_session"] | result["in_ny_session"]
        )
        result["is_high_volume_session"] = result["in_overlap_session"]
        ...
        if (
            asset_type.lower() in ["btc", "bitcoin", "crypto", "eth", "ethereum"]
            and "in_crypto_high" in result.columns
        ):
            result["is_optimal_trading_time"] = (
                result["is_optimal_trading_time"] | result["in_crypto_high"]
            )
            result["is_high_volume_session"] = (
                result["is_high_volume_session"] | result["in_crypto_high"]
            )

        # Add weekday/weekend indicator - vectorized
        result["is_weekend"] = (
            pd.Series(_ts_for_hour.dt.dayofweek, index=result.index) >= 5
        )
...
def analyze_hourly_activity(
    ...
    try:
        ...
        if measure == "volatility" and all(
            ...
            # Group by hour and calculate stats
            hourly_stats = (
                df_copy.groupby("hour")["range"]
                .agg(["mean", "median", "std", "count"])
                .reset_index()
            )

        elif measure == "movement" and all(
            col in df.columns for col in ["open", "close"]
        ):
        ...
        for hour in range(24):
            if (
                hour >= DEFAULT_SESSIONS["asian"]["start"].hour
                and hour < DEFAULT_SESSIONS["asian"]["end"].hour
            ):
                hourly_stats.loc[hourly_stats["hour"] == hour, "session"] = "Asian"
            elif (
                hour >= DEFAULT_SESSIONS["london"]["start"].hour
                and hour < DEFAULT_SESSIONS["london"]["end"].hour
            ):
                hourly_stats.loc[hourly_stats["hour"] == hour, "session"] = "London"
            elif (
                hour >= DEFAULT_SESSIONS["ny"]["start"].hour
                and hour < DEFAULT_SESSIONS["ny"]["end"].hour
            ):
                hourly_stats.loc[hourly_stats["hour"] == hour, "session"] = "New York"

            # Overlap session
            if (
                hour >= DEFAULT_SESSIONS["overlap"]["start"].hour
                and hour < DEFAULT_SESSIONS["overlap"]["end"].hour
            ):
            ...
            if (
                ...
                hour >= CRYPTO_SESSIONS["low_volume"]["start"].hour
                and hour < CRYPTO_SESSIONS["low_volume"]["end"].hour
            ):
                hourly_stats.loc[hourly_stats["hour"] == hour, "crypto_volume"] = "Low"
            else:
                hourly_stats.loc[hourly_stats["hour"] == hour, "crypto_volume"] = (
                    "Medium"
                )
        ...
        hourly_stats.loc[
            hourly_stats["rank"] > 2 * len(hourly_stats) / 3, "recommendation"
        ] = "Avoid"
            ...
            for session_name in sessions:
                ...
                if session_name.startswith("crypto_"):
                    if session_name == "crypto_high":
                        ...
                        session_end = CRYPTO_SESSIONS.get(
                            "high_volume", {"end": time(22, 0)}
                        )["end"]
