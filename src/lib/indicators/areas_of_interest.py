...
def identify_supply_demand_zones(df, zone_length=5, zone_strength_threshold=3, lookforward_window=20):
    ...
    try:
        ...
        if len(result) <= zone_length:
            logger.warning(f"DataFrame too short for zone identification: {len(result)} rows, need at least {zone_length+1}")
            return result
        ...
        for i in range(zone_length, len(result)):
            ...
            touches_mask = ((lookforward_slice['high'] >= zone_low) & 
        ...
        for i in range(zone_length, len(result)):
            ...
            # Vectorized approach for checking touches
            lookforward_slice = result.iloc[i:i+lookforward_window]
            touches_mask = ((lookforward_slice['low'] >= zone_low) & 
...
def is_price_in_area_of_interest(df, current_price=None, buffer_percentage=0.001):
    ...
    try:
        ...
        session_columns = [
            'is_daily_open', 'is_ny_midnight', 'is_session_high', 
            'is_session_low', 'is_weekly_high', 'is_weekly_low'
        ]
        
        for col in session_columns:
            if col in df.columns:
                session_df = df[df[col] == True]
        ...
        result['in_any_area'] = (result['in_fair_value_gap'] or 
                                result['in_supply_zone'] or 
                                result['in_demand_zone'] or 
                                result['at_key_level'] or 
...
def identify_bitcoin_specific_levels(df, fibonacci_base=None):
    ...
    try:
        ...
        for price in halving_prices:
            ...
            is_near_halving_price = (
                (result['high'] >= price - price_buffer) & 
                (result['low'] <= price + price_buffer)
            )
        ...
        if fibonacci_base is not None:
            ...
            for level in fib_levels:
                ...
                is_near_fib_level = (
                    (result['high'] >= level - level_buffer) & 
                    (result['low'] <= level + level_buffer)
                )
