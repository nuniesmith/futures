...
from utils.datetime_utils import apply_datetime_patch, safe_convert_timezone
...
DEFAULT_SESSIONS = config.get_sessions() if hasattr(config, 'get_sessions') else {
...
CRYPTO_SESSIONS = config.get_crypto_sessions() if hasattr(config, 'get_crypto_sessions') else {
    'high_volume': {'start': time(14, 0), 'end': time(22, 0)},
    'low_volume': {'start': time(0, 0), 'end': time(6, 0)}
}

# Get default timezone from configuration
DEFAULT_TIMEZONE = config.get_default_timezone() if hasattr(config, 'get_default_timezone') else 'America/Toronto'


def is_active_session(timestamp: Optional[Union[pd.Timestamp, datetime]] = None, 
                    tz: str = DEFAULT_TIMEZONE, 
                    session_times: Optional[Dict[str, Dict[str, time]]] = None,
                    asset_type: str = 'forex') -> Dict[str, bool]:
    """
    ...
    try:
        ...
        ts = safe_convert_timezone(timestamp, target_tz=tz)
        ...
        if asset_type.lower() in ['btc', 'bitcoin', 'crypto', 'eth', 'ethereum']:
            result.update({
                'in_crypto_high_volume': False,
                'in_crypto_low_volume': False
            })
        ...
        if asset_type.lower() in ['btc', 'bitcoin', 'crypto', 'eth', 'ethereum'] and 'in_crypto_high_volume' in result:
            result['is_optimal_trading_time'] = (result['is_optimal_trading_time'] or 
                                               result['in_crypto_high_volume'])
        ...
        # For crypto, also consider crypto-specific high volume periods
        if asset_type.lower() in ['btc', 'bitcoin', 'crypto', 'eth', 'ethereum'] and 'in_crypto_high_volume' in result:
            result['is_high_volume_session'] = (result['is_high_volume_session'] or 
                                              result['in_crypto_high_volume'])
...
def analyze_session_performance(df: pd.DataFrame, 
    ...
    try:
            ...
            if mask_col in df_copy.columns:
                ...
                if count > 0:
                    ...
                    if 'volume' in session_df.columns:
                        ...
                        avg_volume = 0.0 if pd.isna(avg_volume) else avg_volume
                    else:
                        avg_volume = None
                    ...
                    if 'bullish_mc' in df.columns:
                        bullish_df = session_df[session_df['bullish_mc'] == True]
                        if len(bullish_df) > 0:
                    ...
                    if 'bearish_mc' in df.columns:
                        bearish_df = session_df[session_df['bearish_mc'] == True]
                        if len(bearish_df) > 0:
                            # Setup for success rate calculation
                            success_count = 0
        ...
        if asset_type.lower() in ['btc', 'bitcoin', 'crypto', 'eth', 'ethereum']:
            ...
            if 'avg_volatility' in result_df.columns and 'bullish_mc_success_rate' in result_df.columns:
                ...
                good_sessions = (
                    (result_df['avg_volatility'] > median_vol) & 
...
def should_trade_now(df: Optional[pd.DataFrame] = None, 
                    timestamp: Optional[Union[pd.Timestamp, datetime]] = None,
                    trading_style: str = 'day',
                    asset_type: str = 'forex',
                    respect_weekends: bool = True) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if is_weekend and asset_type.lower() in ['btc', 'bitcoin', 'crypto', 'eth', 'ethereum'] and not respect_weekends:
            is_weekend = False
...
def get_next_session_start(timestamp: Optional[Union[pd.Timestamp, datetime]] = None, 
                         target_session: str = 'london',
                         tz: str = DEFAULT_TIMEZONE) -> pd.Timestamp:
    """
    ...
    try:
        ...
        timestamp_tz = timestamp.tz_convert(tz)
        ...
        if timestamp_tz < session_start_today:
            return pd.Timestamp(session_start_today)
        ...
        return pd.Timestamp(session_start_tomorrow)
...
def get_current_session(timestamp: Optional[Union[pd.Timestamp, datetime]] = None,
                       asset_type: str = 'forex') -> str:
    """
...
def add_session_indicators(df: pd.DataFrame, 
                         time_column: str = 'datetime',
                         asset_type: str = 'forex') -> pd.DataFrame:
    """
    ...
    try:
        ...
        hour_series = pd.Series(time_series.dt.hour, index=result.index)
        ...
        if asset_type.lower() in ['btc', 'bitcoin', 'crypto', 'eth', 'ethereum']:
            if 'in_crypto_high' in result.columns:
        ...
        result['is_weekend'] = pd.Series(time_series.dt.dayofweek, index=result.index) >= 5
...
def analyze_hourly_activity(df: pd.DataFrame, 
                          time_column: str = 'datetime',
                          measure: str = 'volatility') -> pd.DataFrame:
    """
    ...
    try:
        ...
        df_copy = df.copy()
        df_copy['hour'] = time_series.dt.hour
        df_copy['day_of_week'] = time_series.dt.dayofweek
        ...
        if measure == 'volatility' and all(col in df.columns for col in ['high', 'low']):
            ...
            hourly_stats = df_copy.groupby('hour')['volatility'].agg(
            ...
            # Group by hour and calculate stats
            hourly_stats = df_copy.groupby('hour')['range'].agg(
                ['mean', 'median', 'std', 'count']
            ).reset_index()
            ...
            # Group by hour and calculate stats
            hourly_stats = df_copy.groupby('hour')['movement'].agg(
                ['mean', 'median', 'std', 'count']
            ).reset_index()
        ...
        for hour in range(24):
            ...
            # Crypto sessions
            if hour >= CRYPTO_SESSIONS['high_volume']['start'].hour and hour < CRYPTO_SESSIONS['high_volume']['end'].hour:
                hourly_stats.loc[hourly_stats['hour'] == hour, 'crypto_volume'] = 'High'
            elif hour >= CRYPTO_SESSIONS['low_volume']['start'].hour and hour < CRYPTO_SESSIONS['low_volume']['end'].hour:
...
def get_session_calendar(start_date: Union[str, datetime], 
                       end_date: Union[str, datetime],
                       sessions: Optional[List[str]] = None,
                       tz: str = DEFAULT_TIMEZONE) -> pd.DataFrame:
    """
