...
from utils.datetime_utils import apply_datetime_patch
from utils.config_utils import get_config

# Apply datetime patch at module import
apply_datetime_patch()

# Configure logger
logger = logger.bind(name='candle_patterns')


def identify_manipulation_candles(df: pd.DataFrame, trend_period: int = 14, enhanced_detection: bool = False) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if len(df) < trend_period + 1:
            logger.warning(f"DataFrame too small for trend calculation. Need at least {trend_period + 1} rows, found {len(df)}")
...
def get_valid_signals(df: pd.DataFrame, use_countertrend: bool = False, timeframe: str = 'H4',
                    min_strength: float = 0.0, enhanced_filtering: bool = False) -> pd.DataFrame:
    """
...
def generate_entry_signals(df: pd.DataFrame, risk_reward_ratio: float = 2.0, atr_period: int = 14,
                          atr_multiplier: float = 0.5, dynamic_targets: bool = True) -> pd.DataFrame:
    """
    ...
    try:
        ...
        result['tr'] = tr_values
        result['atr'] = result['tr'].rolling(window=atr_period).mean().fillna(result['tr'])
        ...
        if bullish_entries.any():
            ...
            if dynamic_targets:
                ...
                too_far_sl = (result.loc[bullish_entries, 'entry_price'] - result.loc[bullish_entries, 'stop_loss']) > max_distance
                
                # Adjust stop loss if needed
                if too_far_sl.any():
                    result.loc[bullish_entries & too_far_sl, 'stop_loss'] = result.loc[bullish_entries & too_far_sl, 'entry_price'] - max_distance
            else:
                # Standard fixed ATR-based stop loss
                result.loc[bullish_entries, 'stop_loss'] = result.loc[bullish_entries, 'low'] - atr_multiplier * result.loc[bullish_entries, 'atr']
            ...
            for rr in [1.0, 1.5, 2.0, 3.0]:
                ...
                if dynamic_targets and 'candle_height_pct' in result.columns:
                    ...
                    volatile_idx = bullish_entries & (result['candle_height_pct'] > result['candle_height_pct'].median() * 2)
                    if volatile_idx.any():
                        # Adjust targets by up to 15% for highly volatile candles
                        adjustment = 0.85 + (0.15 * (result.loc[volatile_idx, 'candle_height_pct'].median() / 
                                                    result.loc[volatile_idx, 'candle_height_pct']))
                        result.loc[volatile_idx, f'tp_rr_{rr}'] = result.loc[volatile_idx, 'entry_price'] + (risk.loc[volatile_idx] * rr * adjustment)
        ...
        if bearish_entries.any():
            ...
            if dynamic_targets:
                ...
                too_far_sl = (result.loc[bearish_entries, 'stop_loss'] - result.loc[bearish_entries, 'entry_price']) > max_distance
                
                # Adjust stop loss if needed
                if too_far_sl.any():
                    result.loc[bearish_entries & too_far_sl, 'stop_loss'] = result.loc[bearish_entries & too_far_sl, 'entry_price'] + max_distance
            else:
                # Standard fixed ATR-based stop loss
                result.loc[bearish_entries, 'stop_loss'] = result.loc[bearish_entries, 'high'] + atr_multiplier * result.loc[bearish_entries, 'atr']
            ...
            for rr in [1.0, 1.5, 2.0, 3.0]:
                ...
                if dynamic_targets and 'candle_height_pct' in result.columns:
                    # For highly volatile markets, slightly reduce targets for more realistic results
                    volatile_idx = bearish_entries & (result['candle_height_pct'] > result['candle_height_pct'].median() * 2)
                    if volatile_idx.any():
                        # Adjust targets by up to 15% for highly volatile candles
                        adjustment = 0.85 + (0.15 * (result.loc[volatile_idx, 'candle_height_pct'].median() / 
                                                    result.loc[volatile_idx, 'candle_height_pct']))
        ...
        result['reward_distance'] = abs(result['entry_price'] - result[f'tp_rr_2.0'])
        with np.errstate(divide='ignore', invalid='ignore'):
            result['risk_reward_ratio'] = np.where(
                result['risk_distance'] > 0,
                result['reward_distance'] / result['risk_distance'],
                np.nan
            )
...
def identify_advanced_patterns(df: pd.DataFrame, lookback_period: int = 5) -> pd.DataFrame:
    ...
    try:
        ...
        if not all(col in df.columns for col in required_columns):
            logger.error(f"Missing required OHLC columns")
            return df
        ...
        if len(result) < lookback_period + 2:
            logger.warning(f"DataFrame too small for pattern identification. Need at least {lookback_period + 2} rows, found {len(result)}")
            return result
        ...
        if len(result) >= 4:
            ...
            highs = result['high'].rolling(window=4).apply(lambda x: (x[1] > x[0] and x[2] > x[1] and x[3] < x[2]), raw=True)
            result['three_pushes_top'] = highs
            
            # Same for lows (three consecutive lower lows followed by reversal)
            lows = result['low'].rolling(window=4).apply(lambda x: (x[1] < x[0] and x[2] < x[1] and x[3] > x[2]), raw=True)
...
def get_pattern_strength(df: pd.DataFrame, index: int, pattern_weights: Optional[Dict[str, float]] = None) -> Dict[str, float]:
    """
    ...
    try:
        ...
        if 'trend' in row:
            if row['trend'] == 'up' and any(p.startswith('bullish') for p in patterns):
                strength += 0.2  # Bonus for trend-aligned patterns
            elif row['trend'] == 'down' and any(p.startswith('bearish') for p in patterns):
                strength += 0.2  # Bonus for trend-aligned patterns
...
def filter_signals_for_crypto(df: pd.DataFrame, min_strength: float = 0.6, 
    ...
    try:
        ...
        if not result['entry_signal'].any():
            return result
        
        # Get config for crypto
        config = get_config()
        btc_settings = config.get_asset_settings('btc')
        
        # Apply strength filtering if signal_strength column exists
        initial_count = result['entry_signal'].sum()
        ...
        if respect_key_levels and 'key_level' in result.columns:
            ...
            non_key_signals = ~result['key_level'] & result['entry_signal']
            ...
            if aoi_columns:
                ...
                aoi_mask = result[aoi_columns].any(axis=1)
                non_key_signals = ~aoi_mask & non_key_signals
        ...
        if 'candle_height_pct' in result.columns:
            ...
            low_vol_signals = (result['candle_height_pct'] < result['candle_height_pct'].median() * 0.5) & result['entry_signal']
        ...
        logger.info(f"Crypto filtering removed {removed_count} of {initial_count} signals ({removed_count/initial_count*100:.1f}%)")
