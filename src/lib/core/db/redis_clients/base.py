...
import random
import threading
from typing import Optional, Callable, TypeVar, ParamSpec, Concatenate, TYPE_CHECKING, Dict, Any, Union, List, Tuple, ClassVar
...
def _ensure_connection(method: Callable[Concatenate[RedisClientType, P], R]) -> Callable[Concatenate[RedisClientType, P], R]:
    """
    ...
    def wrapper(self_instance: RedisClientType, *args: P.args, **kwargs: P.kwargs) -> R: # Use RedisClientType here
        ...
        logger.debug(f"{wrapper_log_prefix} Checking Redis connection status. Current connection: {self_instance.connection} (Type: {type(self_instance.connection)})")
        if not self_instance.connection or not self_instance._check_connection_health(timeout=1):
            logger.warning(f"{wrapper_log_prefix} ⚠️ Redis connection is lost or not responding. Attempting to reconnect...")
...
class BaseRedisClient(ABC):
    ...
    def __init__(
        ...
        logger.debug(f"{log_prefix} START - Initializing BaseRedisClient with Timeout: {timeout}s, Max retries: {max_retries}, Max Connections: {max_connections}.")
        self.max_connections = max_connections
        self.timeout = timeout
        self.max_retries = max_retries
        self.connection_kwargs = connection_kwargs or {}
        self.last_connection_attempt = 0  # For throttling reconnection attempts
        self.connection: Optional[redis_clients.Redis] = None
    ...
    def _setup_connection_params(self):
        ...
        from src.lib.core.db.redis_clients.utils import construct_redis_url, clean_redis_url
    ...
    def _get_connection_pool_key(self) -> str:
        ...
        if self.connection_params.get("use_sentinel", False):
            ...
            key_parts.extend([
                f"cluster:{','.join(hosts)}",
                f"port:{self.connection_params.get('cluster_port', '6379')}"
            ])
        else:
            key_parts.extend([
    ...
    def _check_connection_health(self, timeout: Optional[float] = None) -> bool:
        """
