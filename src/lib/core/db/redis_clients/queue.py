...
class RedisQueue:
    ...
    def __init__(self, redis_client: "RedisClient", queue_name: str = "training_jobs"):
        ...
        """
        # Validate redis_client is not a dict or None
        ...
        if self.use_async:
            ...
            async def _async_dequeue():
                try:
                    ...
                    try:
                        job_id = await self.redis.rpoplpush(self.pending_queue, self.processing_queue)
                        if not job_id:
                            return None
                        ...
                        job_id_future = await self.redis.rpop(self.pending_queue)
