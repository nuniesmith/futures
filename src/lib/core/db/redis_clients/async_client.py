...
from typing import Any, Optional, Tuple, cast, Union, Callable, Iterable
...
class AsyncRedisClient(BaseRedisClient):
    ...
    def _create_connection(self) -> redis_async.Redis:
        ...
        if self.connection_params.get("use_cluster", False):
            # For cluster mode
            from redis.asyncio.cluster import RedisCluster, ClusterConnectionPool
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                if self.connection and await self.ping():
                    ...
                    logger.debug(f"{log_prefix} END - Async connection initialization SUCCESS on attempt {attempt_num}.")
                    return
            except redis_clients.RedisError as e:
                logger.warning(f"{log_prefix} ⚠️ Redis connection failed on attempt {attempt_num}/{self.max_retries}: {e}")
    ...
    async def get(self, key: str) -> Optional[Any]:
        ...
        try:
            if self.connection is None:
                ...
                logger.error(f"{log_prefix} ❌ {error_message}")
                logger.debug(f"{log_prefix} END - ASYNC GET operation FAIL - No connection available. Raising RedisError.")
                raise RedisError(error_message)
            ...
            logger.error(f"{log_prefix} ❌ Connection error during ASYNC GET operation for key '{key}': {e}", exc_info=True)
            await self._initialize_connection_async()
            logger.debug(f"{log_prefix} END - ASYNC GET operation ERROR - Connection error. Reconnection attempted. Returning None.")
            return None
    ...
    async def set(self, key: str, value: Any, expiry: Optional[int] = None) -> bool:
        ...
        logger.debug(f"{log_prefix} START - ASYNC SET operation for key: '{key}', expiry: {expiry}. Value type: {type(value)}.")
        ...
        try:
            ...
            if self.connection is None:
                error_message = "Redis connection is not established."
                logger.error(f"{log_prefix} ❌ {error_message}")
                logger.debug(f"{log_prefix} END - ASYNC SET operation FAIL - No connection available. Raising RedisError.")
                raise RedisError(error_message)
    ...
    async def delete(self, key: str) -> bool:
        ...
        try:
            if self.connection is None:
                error_message = "Redis connection is not established."
                logger.error(f"{log_prefix} ❌ {error_message}")
                logger.debug(f"{log_prefix} END - ASYNC DELETE operation FAIL - No connection available. Raising RedisError.")
                raise RedisError(error_message)
    ...
    async def exists(self, key: str) -> bool:
        ...
        try:
            if self.connection is None:
                error_message = "Redis connection is not established."
                logger.error(f"{log_prefix} ❌ {error_message}")
                logger.debug(f"{log_prefix} END - ASYNC EXISTS check FAIL - No connection available. Raising RedisError.")
                raise RedisError(error_message)
    ...
    async def keys(self, pattern: str = "*") -> list:
        ...
        try:
            if self.connection is None:
                error_message = "Redis connection is not established."
                logger.error(f"{log_prefix} ❌ {error_message}")
                logger.debug(f"{log_prefix} END - ASYNC KEYS retrieval FAIL - No connection available. Raising RedisError.")
                raise RedisError(error_message)
    ...
    def pipeline(self, transaction: bool = True) -> Pipeline:
        ...
        if self.connection is None:
            ...
            logger.error(f"{log_prefix} ❌ {error_message}")
            logger.debug(f"{log_prefix} END - ASYNC Pipeline retrieval FAIL - No connection available. Raising RedisError.")
            raise RedisError(error_message)
    ...
    async def zrange(
        ...
        logger.debug(f"{log_prefix} START - ASYNC ZRANGE operation for key: '{key}', start: {start}, end: {end}, desc: {desc}, withscores: {withscores}, score_cast_func provided: {score_cast_func is not None}.")
        ...
        try:
            ...
            if score_cast_func is None:
                ...
                result_raw = await self.connection.zrange(key, start, end, desc=desc, withscores=withscores, score_cast_func=score_cast_func)
    ...
    async def get_last_fetched_timestamp(self, asset: str, timeframe: str) -> Optional[int]:
        """
    ...
    async def calculate_fetch_range(
        ...
        if start_timestamp >= end_timestamp:
            logger.debug(f"{log_prefix} No new data needed. start_timestamp ({start_timestamp}) >= end_timestamp ({end_timestamp}).")
    ...
    async def hset(self, name: str, key: str, value: Any) -> bool:
        ...
        logger.debug(f"{log_prefix} START - ASYNC HSET operation for hash: '{name}', key: '{key}'. Value type: {type(value)}.")
    ...
    async def setex(self, name: str, time: int, value: Any) -> bool:
        ...
        logger.debug(f"{log_prefix} START - ASYNC SETEX operation for key: '{name}', expiry: {time}s. Value type: {type(value)}.")
