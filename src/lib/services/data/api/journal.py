...
_JOURNAL_BODY = """\
<div style="background:var(--bg-panel);border:1px solid var(--border-panel);border-radius:12px;
            padding:1.25rem;min-height:calc(100vh - 90px)">
  <!-- Journal fragment — loaded immediately on page load.
       hx-swap="innerHTML" keeps the #journal-panel-inner wrapper in the DOM
       so that subsequent HTMX swaps from within the fragment still work. -->
  <div id="journal-panel-inner"
       hx-get="/journal/html"
       hx-trigger="load"
       hx-swap="innerHTML">
