...
logger = get_logger("wave_analysis")
...
def _build_ticker_to_asset_name() -> dict[str, str]:
    ...
    """
    # Contract spec modules (KRAKEN_CONTRACT_SPECS, MICRO_CONTRACT_SPECS) have
    # been removed.  Return an empty map — callers fall back to heuristic
    # matching against ASSET_PARAMS keys when no ticker mapping is found.
    return {}
...
def resolve_asset_params(
    ...
    if any(
        probe.startswith(p)
        for p in ("KRAKEN:", "BTC", "ETH", "SOL", "AVAX", "LINK", "DOT", "ADA", "XRP")
    ):
    ...
    if any(
        probe.startswith(p)
        for p in ("MES", "MNQ", "M2K", "MYM", "ES", "NQ", "RTY", "YM")
    ):
        # Equity index
...
def calculate_wave_analysis(
    ...
    # Step 2: Detect waves
    bull_changes, bear_changes, _bull_durations, _bear_durations = _detect_waves(
        close, dyn_ema
    )
